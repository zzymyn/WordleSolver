...
namespace WordleSolver // Note: actual namespace depends on the project name.
{
    internal class Program
    {
        ...
        static readonly Word[] AllWords = new Word[]
        {
        ...
        static readonly Word[] AllAnswers = new Word[]
        {
        ...
        {
            var allValues = AllWords.Select(guess =>
            {
                var guessValue = AllAnswers.AsParallel().Max(answer =>
                {
                    ...
                    if (mmr.Any())
                    {
                        //Console.WriteLine($"{guess},{answer} = {mmr.Max()}");
                        return mmr.Max();
                    }
                    else
                    {
                ...
                Console.WriteLine($"{guess} = {guessValue}");
                return guessValue;
            }).ToList();
            ...
            foreach (var (guess, guessValue) in allValues.Zip(AllWords).OrderBy(a => a.First).Take(10))
            {
                Console.WriteLine($"{guess} = {guessValue}");
            }
        }
