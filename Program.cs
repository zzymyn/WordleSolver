...
namespace WordleSolver // Note: actual namespace depends on the project name.
{
    internal class Program
    {
        ...
        static int Main(string[] args)
        {
            ...
            List<int> allValues = RunLevel(testWords, Level, "");

            Console.WriteLine();
            Console.WriteLine("BEST WORDS:");

            foreach (var (guess, guessValue) in allValues.Zip(testWords).OrderBy(a => a.First).Take(10))
            {
                Console.WriteLine($"{guessValue}: {guess}");
            }

            return 0;
        }

        private static List<int> RunLevel(List<Word> testWords, int level, string logPrefix)
        {
            var allValues = testWords.Select(guess =>
            {
                var guessValue = AllAnswers.AsParallel().Max(answer =>
                {
                    //var answer = GetAnswer(guess, solution);
                    var next = RemoveCandidates(AllWords, guess, answer);

                    if (level <= 1)
                        return next.Count();

                    var max = RunMinMax(next, guess, answer, level - 1, logPrefix);
                    if (Verbosity >= 2)
                    {
                        Console.WriteLine($"{logPrefix}{guess}={answer}: {max}");
                    }
                    return max;
                });
                ...
                if (Verbosity >= 1)
                {
                    Console.WriteLine($"{logPrefix}{guess}=*: {guessValue}");
                }
        ...
        private static int RunMinMax(List<Word> next, Word guess, Word answer, int level, string logPrefix)
        {
            ...
            var allValues = RunLevel(next, level - 1, $"{logPrefix}{guess}={answer},");
